...
This will include the appropriate `"main"` (`umd`), `"module"` (`esm`), `"jsnext:main"` (`esm`) and `"browser"` (`dist` minified) properties in the `package.json`. It will also update the `"build"` script to pass the matching argument.
...
This will update the appropriate `"main"` (`umd`), `"module"` (`esm`), `"jsnext:main"` (`esm`), `"browser"` (`dist` minified), and `"types"` (Typescript) properties in the `package.json`. It will also update the `"build"` script to pass the matching argument.
...
Runs the lib's tests in on-going watch mode during active development.
...
```sh
benmvp build --formats esm umd
```

To put ESM & web distribution formats in an alternate build location:

```sh
benmvp build -f esm dist -o ./built
```
